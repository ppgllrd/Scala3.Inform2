...
def quickSort[A](xs: Array[A])(using ord: Ordering[A]): Unit =
  ...
  def partition(left: Int, right: Int): Int =
    ...
    swap(pivotIndex, right) // place pivot at the end
...
// tests quickSort with a number of random arrays
@main def quickSortCorrectness(): Unit =
  ...
  for seed <- 0 until numTests do
    ...
    val correct = isSorted(ys) && sameElements(xs, ys)

    if !correct then
      sys.error(s"quickSort algorithm failed")

  // if we reach this point without an error, then we passed all tests
  println("quickSort passed all sorting tests")
