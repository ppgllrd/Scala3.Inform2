...
object Stroke:
   ...
   * @param miterlimit the limit to trim the miter join. The miterlimit must be greater than or equal to 1.0.
  ...
  def unapply(s: Stroke): (Double, Int, Int, Double, Array[Double], Double) =
    (s.getLineWidth.toDouble, s.getEndCap, s.getLineJoin, s.getMiterLimit
      , s.getDashArray.map(_.toDouble), s.getDashPhase.toDouble)